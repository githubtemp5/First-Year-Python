...
def getInputs():
    ...
    while inputPatchDimension not in validDimensions :
    ...
    for counter in range(3):
        ...
        while(inputColour not in validColour or inputColour in colours):
            ...
            print("Invalid colour or colour already entered. Valid colours: ")
...
def drawPatches(win, patchDimension, colours):
    ...
    counter = patchDimension//100-1
    
    #defining intial colours
    colourForCurrentRow = []
    
    for counter in range(patchDimension//100):
        colourForCurrentRow.append(0)
        
    currentColumn = patchDimension//100 -1
    ...
    for y in range(0, patchDimension, 100):
        ...
        for change in range(currentColumn, patchDimension//100, 1):
        ...
        for x in range(0, patchDimension, 100):
          #draws the patch and stores colour, start point and the shapes in a list  
            if( x//100 in patchIICoor):
                patchII.append(drawPatchII(win, colours[colourForCurrentRow[x//100]], x, y)) 
                patchIIColour.append(colourForCurrentRow[x//100])
                patchIIStartPoints.append(Point(x, y))
                
            else:
                tempPatchIFill, tempPatchIOutline = drawPatchI(win, colours[colourForCurrentRow[x//100]], x, y)
                patchIFill.append(tempPatchIFill)
                patchIOutline.append(tempPatchIOutline)
                patchIColour.append(colourForCurrentRow[x//100])
    ...
    while(True):
        ...
        clickedStartX = correctBoundary((clickedPoint.getX()//100)*100, patchDimension)
        clickedStartY = correctBoundary((clickedPoint.getY()//100)*100, patchDimension)
...
def calculateNextColour(currentCol):
    ...
    # returns the position of clicked patch from the list
...
#if the click is more than the patch dimension it gets it back to the correct size.
...
# horizontal and vertical lines patch.
...
def createWindow(patchDimension):
    win = GraphWin("Coursework", patchDimension, patchDimension)
